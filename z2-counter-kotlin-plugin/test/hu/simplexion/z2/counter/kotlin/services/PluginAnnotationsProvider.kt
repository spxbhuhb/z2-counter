...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    ...
    companion object {
        private const val ANNOTATIONS_JAR_DIR = "../z2-counter-runtime/build/libs/"
        private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name ->
            name.startsWith("z2-counter-runtime-jvm") && name.endsWith(".jar") && "sources" !in name
        }
    }
    ...
    private val failMessage = { "Jar with annotations does not exist. Please run :z2-counter-runtime:jar" }
}